...
\begin{example}
\label{D!lowrank_approx:example}
...
\subsection{Related works on Morse theory of distance functions}
\label{D!sec:related}
...
\begin{proof}
  The fact that $M_Y$ is definable is obvious. The fact that it has codimension at least one follows from the strict convexity of the norm and the fact that $\dist_Y$ is continuously differentiable on $\R^n \setminus (Y \cup \overline {M_Y}))$ is the content of \cite[Theorem 4.8(5)]{Federer1959}.
\end{proof}
...
\begin{remark}
  More generally, an analogue of the previous proposition still holds true for a proper definable function $f \colon X\to \R$ (without the assumption that it is locally Lipschitz). 
...
\begin{proof}
  This follows from \cite[Sect.~4]{APS1997}, analyzing the conditions of \cite[Theorem 4.2]{APS1997} case by case.
\end{proof}
...
\begin{proof}
  As in the classical case, the first equality follows from \cref{D!propo:strongMorse} by comparing the strong Morse inequalities for large enough values $\lambda$ and $\lambda +1$. The second equality follows from the first one applied to the pairs $(X,Y)$ and $(Y,X)$.
\end{proof}

\begin{remark}
  The conclusions of \cref{D!propo:strongMorse} are still valid if $X=\R^n$ and $Y$ is compact, since this guarantees that the distance function from $Y$ is proper.
\end{remark}
...
\begin{example}[Distance function from a finite set, continuation of \cref{D!ex:finiteY}]
\label{D!example:points2}
...
\label{D!T:Yomdin}
  For the generic $q \in \Poly_d$ with $d\geq 2$, writing $Y = Z(q)$,  for every $x\in\R^n$ the set $B(x, \dist_Y(x)) \cap Y$ is a nondegenerate simplex with at most $n + 1$ points.
\end{theorem}
...
\label{D!T:finite-crit-pt}
  ...
  for some $c(k,n)>0$ depending on $n$ and $k$ only.
\end{theorem}
