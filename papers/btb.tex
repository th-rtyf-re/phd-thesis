...
\begin{remark}
The decomposition theorem \cite{CrawleyBoevey2015,BotnanCrawleyBoevey2020} states that every persistence module  $X$ in $\Pfd$ admits a barcode decomposition  $\varphi_X \colon X \cong \bigoplus_i K(a_i, b_i)$. While the isomorphism $\varphi_X$ is not unique
% \ir{(give an example)}
...
% \item For all $t \in \R$, every same-start-point block of $\Phi_t$ (the matrix $\Phi$ restricted to rows and columns whose label contains $t$) is invertible.
% \item For all $t \in \R$, every same-start-point block of $\tilde{\Phi}_t$ is invertible.
% \item For all $t \in \R$, $\tilde{\varphi}_t$ is an isomorphism.
% \item $\tilde{\varphi}$ is an isomorphism.
% \end{itemize}
% Thus $\tilde{\varphi}$ is an isomorphism. We reason similarly for $\tilde{\psi}$, ordering the matrix labels by death than birth, and using same-endpoint blocks instead of same-start-point blocks. We deduce that $\tilde{g}$ and $\tilde{h}$ are isomorphic.

% Similarly to \cref{C!P:unique-induced-matching}, we can view the morphisms $\tilde{g}$ and $\tilde{h}$ as objects in the category $\Fun(\R \times \{0 < 1 \}, \Vect_K)$. Applying \cref{C!A:labeled-red}, we deduce that these objects decompose into simple indecomposables (compare also with \cite[Theorem 5.3]{JNT2023}). As this decomposition is unique, we conclude that the pivots of $\tilde{G}$ and $\tilde{H}$ are the same up to permutation of identically labeled bars.
...
% \begin{itemize}
% \item In the first while loop, column operations only happen between columns with the same start-point. Indeed, the block in $\Phi$ whose rows and columns are labeled with a given start-point is a diagonal block of $\Phi$. Since $\Phi$ is invertible, this block must be of full rank, and so the lowest nonzero element of a column in this block must belong to a row in the block. Moreover, no column to the left of this block may have a lowest nonzero coefficient on a row of this block, as that column's label would not overlap above this block's rows' labels. In this block, the end-points are in increasing order, so all operations are valid: in particular, nonzero coefficients are always between overlapping bars.
% \item In the second while loop, by design, the row operations only happen between rows with the same end-point. Once again, since the start-points in this block are in increasing order, all operations are valid.
...
%   }
%   $i \leftarrow \low(j)$\;
%   $i_0 \leftarrow \min\{i' \in I \mid a_{i'} = a_i\}$ \tcp*{highest row with same start-point}
