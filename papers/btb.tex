...
% \documentclass[numberwithinsect, cleveref, autoref]{lipics-v2021}
% \nolinenumbers
% % \usepackage[scale=.75]{geometry} % scale is the portion of the paper that is used. Default is 0.7
% \usepackage{graphicx}
% \usepackage{caption}
...
% \usepackage{mathtools,amssymb,stmaryrd}\allowdisplaybreaks
% % \usepackage[libertine]{newtxmath}
% % \newcommand\hmmax{0} % reduce number of fonts imported by bm
% % \usepackage{bm}
% \usepackage{booktabs}

%%%% LISTS
% \usepackage{enumitem}
% \setlist[itemize]{label=\textbullet, topsep=1ex, itemsep=0ex, leftmargin=5ex}
% \setlist[enumerate]{topsep=1ex, itemsep=0ex}
...
% \newtheorem{todo}[subsection]{To do}
...
% \newcommand{\bigboxplus}{\mathop{\scalebox{1.5}{$\boxplus$}}\displaylimits}
% \usepackage{scalerel}
% \makeatletter
% \DeclareMathOperator*{\bigboxplus}{\scalerel*{\ooalign{$\m@th\boxempty$\cr\hidewidth$\m@th+$\hidewidth\cr}}{\bigoplus}}
% \makeatother



% \newcommand{\ir}[1]{{\color{magenta} #1}}
% \newcommand{\ag}[1]{{\color{teal} #1}}

% ==== Metadata ====

% \title{Barcode matchings and \texorpdfstring{$p$}{p}-norms of persistence modules}
% % REFERENCE: \author{name}{affil}{email}{orcid}{funding}
% \author{Andrea Guidolin}{University of Southampton}{}{}{}
% \author{Arthur Reidenbach}{ENS Paris--Saclay}{}{}{}
% \author{Isaac Ren}{KTH Royal Institute of Technology}{}{}{}
% \author{Martina Scolamiero}{KTH Royal Institute of Technology}{}{}{}
...
\end{equation}
where each $f_{ij}$ is a nonzero morphism $K(a_i, b_i) \to K(c_j, d_j)$, each $g_i$ denotes the zero morphism $K(a_i, b_i) \to 0$ and each $h_j$ denotes the zero morphism $0 \to K(c_j, d_j)$. In \cite{AGRS2025}, the authors (some of whom are authors of the current paper) call a morphism \textbf{bar-to-bar} if it admits a decomposition as in \eqref{C!E:intro-btb}. Such bar-to-bar morphisms can be seen as an algebraic version of barcode matchings; indeed, $\mu$ is the corresponding matching. The authors then induce bar-to-bar morphisms from mono- and epimorphisms, and recover an isometry result between algebraic and combinatorial Wasserstein distances as first described in \cite{skraba2020wasserstein}.

\subsection{Our contributions}

In this paper, we continue the analysis of \cite{AGRS2025} by studying the matching induced by bar-to-bar morphisms. Following the strategy of \cite{bauer2015induced}, we induce a barcode matching for an arbitrary morphism $f \colon X \to Y$ by first finding an epi-mono factorization
\[
f \colon X \xrightarrow{e} \im(f) \xrightarrow{m} Y
\]
where $\im(f)$ is the image of $f$, $e \colon X \to \im(f)$ is an epimorphism, and $m \colon \im(f) \to Y$ is a monomorphism. We then compute the induced matchings of $e$ and $m$ via their induced bar-to-bar morphisms, and then compose them together to obtain the induced matching of $f$. Note that this depends \textit{a priori} on the choice of epi-mono factorization; in practice, this is not the case for the induced matching of \cite{bauer2015induced} but is the case for our matchings (\ref{C!ex:counterex2}).

The key insight of this paper is that we can induced bar-to-bar morphisms by projecting morphisms down to a subspace of morphisms that only map nontrivially between bars with the same endpoint: this is the \textbf{same-end operator}, denoted by $\Delta$ (\cref{C!D:Delta}). Given a morphism $f$, $\Delta f$ is a direct sum of morphisms between persistence modules with no nested bars. We prove, in the p.f.d.\ setting, that these morphisms are bar-to-bar, which implies that $\Delta f$ is bar-to-bar (\cref{C!T:Delta-is-btb}). As described above, this naturally induces a barcode matching $\mu_\Delta(f)$ (\cref{C!D:Delta-matching}). In particular, this matching is additive: given morphisms $f$ and $g$, $\mu_\Delta(f \oplus g) = \mu_\Delta(f) \oplus \mu_\Delta(g)$.

Our proof techniques rely on matrix representations of morphisms. To this end, we introduce the notion of labeled matrices and their multiplication (\cref{C!D:labeled-matrix}). In particular, to show that $\Delta f$ is bar-to-bar, we reduce a matrix representation of $\Delta f$ to a matrix with at most one nonzero coefficient in each row and column (\cref{C!A:inf-red}. The induced matching is then read from the pivots of this reduced matrix representation.
...
This definition is adapted from \cite[\textsection 2.3]{bauer2015induced}.
...
Let $a, b, c, d \in \E$. Following \cite{bauer2020persistence}, we say that \textbf{the interval $\langle a, b \rangle$ overlaps the interval $\langle c, d \rangle$ above} (and antisymmetrically, $\langle c, d \rangle$ \textbf{overlaps $\langle a, b \rangle$ below}) if $c \leq a < d \leq b$.
\end{definition}

\begin{definition}[Interval modules]
Let $a < b$ be elements of $\E$.
A \textbf{bar} (or \textbf{interval module}) with \textbf{start-point} $a$ and \textbf{end-point} $b$, denoted as $K(a, b)$, is the persistence module defined as follows: for any $u$ in $\R$,
\begin{equation*}
K(a, b)_u \coloneq
\begin{cases}
...
In order to better manage these interval modules, we will be considering the following subcategory of $\Pfd$. Recall that a \textbf{multiset} is a set $S$ equipped with a \textbf{multiplicity} $m_S \colon S \to \N$, and that when there is no ambiguity, $S$ and $m_S$ may be used interchangeably.


\begin{definition}[Subcategory of bars]
We denote by $\Bar$ the full subcategory of $\Pfd$ whose objects are the (potentially infinite) direct sums of bars $\bigoplus_{i \in I} K(a_i, b_i)$. We call this the \textbf{category of bars}.

Given an object $X = \bigoplus_{i \in I} K(a_i, b_i)$ of $\Bar$, we define its \textbf{barcode} as a representation of the multiset $\cc{B}(X) \coloneq \{\langle a_i, b_i \rangle\}_{i \in I}$ of intervals appearing in $X$ (recall the definition of multiset representations from \cite{bauer2020persistence}).
\end{definition}

\begin{definition}
\label{C!D:barcode-decomp}
Let $X$ be a persistence module in $\Pfd$. A \textbf{barcode decomposition} of $X$ is the given of an object $\bigoplus_i K(a_i, b_i)$ in $\Bar$ and an isomorphism $\varphi_X \colon X \cong \bigoplus_i K(a_i, b_i)$.
\end{definition}

\begin{remark}
The decomposition theorem \cite{crawley2015decomposition,botnan2020decomposition} states that every persistence module  $X$ in $\Pfd$ admits a barcode decomposition  $\varphi_X \colon X \cong \bigoplus_i K(a_i, b_i)$. While the isomorphism $\varphi_X$ is not unique
% \ir{(give an example)}
, the direct sum of bars $\bigoplus_i K(a_i, b_i)$ is, up to reordering of the bars. In particular, we can speak of the \textbf{barcode} of an object in $\Pfd$ as well.  
\end{remark}

\begin{remark}
\label{C!R:ephemeral}
Every persistence module in $\Pfd$ has at most countably many bars $K(s, t)$ with $s < t \in E$ in its barcode decomposition. In other words, up to an \textbf{ephemeral} summand (see \cite{Chazal2012TheSA}), which is a direct sum of bars of the form $K(s^-, s^+)$ supported on $\{s\}$, every persistence module $X$ in $\Pfd$ has at most countably many bars. 
...
As a notion of \emph{size} for persistence modules, we use the $p$-norm first introduced in \cite{p-norm_Edesbrunner} and \cite{skraba2020wasserstein}, also studied in \cite{AGRS2025} for tame persistence modules.  

\begin{definition}
\label{C!def:pnorm}
For $p\in [1,\infty]$, the \textbf{$p$-norm} of a persistence module with barcode decomposition $X\cong \bigoplus_{i} K(a_i, b_i)$ is:
\begin{equation*}
...
\end{equation*}
\end{definition}
By Remark \ref{C!R:ephemeral}, the $p$-norm of a persistence module in $\Pfd$ involves an infinite sum with at most countably many positive summands. Following \cite{skraba2020wasserstein} (Definition $2.11$) we say that a persistence module $X$ has \emph{bounded p-norm} if $\left\| X\right\|_{p}^p<\infty$.

\subsection{Morphisms and matrices}\label{C!sec:mor-mtrx}
...
Let $A \in K^{I \times J}$ and $B \in K^{J \times L}$ be labeled, column-finite matrices where the columns of $A$ and the rows of $B$ have the same labels. The \textbf{overlap multiplication} of $A$ and $B$, written as the usual product $AB$, is defined by, for all $(i, \ell) \in I \times L$,
\begin{equation}
...
\item When every interval indexed by $L$ overlaps every interval indexed by $I$, overlap multiplication corresponds exactly to standard matrix multiplication. For example, this is the case when every interval has the same endpoint.
\end{enumerate}
\end{remark}

\begin{definition}[Matrix representations of bar morphisms]
\label{C!D:matrix}
Let $f \colon \bigoplus_{j \in J} K(a_j, b_j) \to \bigoplus_{i \in I} K(c_i, d_i)$ be a morphism in $\Bar$ (note the swapped indices). An \textbf{ordered matrix representation of $f$} is a labeled matrix $(F_{ij})_{i \in I, j \in J}$ where
\begin{enumerate}
\item for all $i \in I$ and $j \in J$, the entry $F_{ij}$ is the projection of $f$ to $\hom(K(a_j, b_j), K(c_i, d_i))$ via the canonical isomorphisms \eqref{C!E:iso1} and \eqref{C!E:iso2},
\item $I$ and $J$ are ordered in a way compatible with the product order on the pairs $(a_j, b_j)$ and $(c_i, d_i)$, and
\item row $i$ is labeled by $\langle c_i, d_i \rangle$ and column $j$ is labeled by $\langle a_j, b_j \rangle$.
\end{enumerate}
\end{definition}
...
\[
    F = \bordermatrix { 
    & \langle a_1,b_1 \rangle & \langle a_2,b_2 \rangle \cr 
    \langle c_1,d_1 \rangle & 1 & 1 \cr
    \langle c_2,d_2 \rangle & 1 & 0 \cr}
\]
\end{example}
...
Set-theoretic \textbf{matchings} are defined as partial injective functions between sets \cite{bauer2020persistence}, but we will use the following equivalent definition in order to more easily generalize to multiset matchings.

\begin{definition}[Matchings]
Let $I$ and $J$ be sets. A (\textbf{set-theoretic partial}) \textbf{matching between $I$ and $J$} is a subset $\mu \subseteq I \times J$ such that the projections $\pi_I$ and $\pi_J$ onto $I$ and $J$, respectively, are injective. We denote by $\Match(I, J)$ the set of matchings between $I$ and $J$.
\end{definition}

We use the definitions of the category $\mathbf{Mch}$ (including composition of matchings $\circ$), \textbf{multiset representations}, \textbf{barcodes}, and the category $\Barc$ (including the overlap composition of matchings $\bullet$) from \cite{bauer2020persistence}. We also define multiset matchings:

\begin{definition}[Multiset matchings {\cite[Definition 2.4]{GST2025}}]
Let $(S, m_S)$ and $(T, m_T)$ be two multisets. A (\textbf{partial multiset}) \textbf{matching between $S$ and $T$} is a multiset $(M, m_M)$ where $M = S \times T$ and, for all $s \in S$ and $t \in T$,
\[
...
Let $S$ and $T$ be multisets of intervals of the real line, $M \in \Match(S, T)$ a matching, and $p \in [1, \infty]$. The \textbf{$p$-cost of $M$} is
\[
...
In this paper, we will mainly prove results for monomorphisms in $\Pfd$. Analogous results hold for epimorphisms by a duality argument. Specifically, we consider the contravariant dualization endofunctor $(\cdot)^* \colon \Pfd \to \Pfd$ defined by \cite[Section 3.4]{bauer2015induced}, which send a map $f \colon X \to Y$ to the \textbf{dual map} $f^* \colon Y^* \to X^*$. In particular, this functor it sends epimorphisms to monomorphisms, and vice versa. It also sends the bar $K(a, b)$ to the bar $K(-b, -a)$, where the negative sign acts on the decorated real line $\bb{E}$ by $-(t^\tau) = (-t)^\tau$.

We also define the dual of a matching as follows.

\begin{definition}
Let $X$ and $Y$ be persistence modules in $\Pfd$ and $\mu \in \Match(\cc{B}(X), \cc{B}(Y))$ be a matching of barcodes. The \textbf{dual of $\mu$} is the matching $\mu^*$ in $\Match(\cc{B}(Y^*), \cc{B}(X^*))$ defined by
\[
...
\begin{definition}
\label{C!D:btb}
A morphism $f \colon X \to Y$ of persistence modules in $\Bar$ is \textbf{strictly bar-to-bar}
...
A morphism $f \colon X \to Y$ in $\Pfd$ is \textbf{bar-to-bar} if there exist barcode decompositions of $X$ and $Y$ such that the associated morphism in $\Bar$ is strictly bar-to-bar. In other words, there exist barcode decompositions $(\varphi_X,\bigoplus_i K(a_i,b_i))$, $(\varphi_Y,\bigoplus_j K(c_j,d_j))$ of $X$ and $Y$ respectively and $g \colon \bigoplus_i K(a_i,b_i) \rightarrow \bigoplus_j K(c_j,d_j)$ strictly bar-to-bar such that the following diagram commutes:
\[
\begin{tikzcd}
...
\begin{figure}
\centering
\begin{tikzpicture}[scale = 0.4]%, baseline = {([yshift = .5ex](0, 2))}]
\draw[line width=1pt]
...
\]
where the indecomposables $\bf{R}^*_*$ and $\bf{I}^*_*$ are adapted from \cite[Definition~5.2]{jacquard2023space} and defined as follows:
\begin{itemize}
...
\end{itemize}
By the Krull-Remak-Schimdt-Azumaya theorem \cite{Goro1950} (see also \cite{botnan2020decomposition}), this decomposition is unique up to permutation of the indecomposables. This implies that the matching $\mu$ is unique up to permutations of indices $i, i'$ in $I$ such that $a_i = a_{i'}$ and $b_i = b_{i'}$, and similarly for $J$. In particular, the induced matching $M$ on barcodes is unique. The fact that $M$ is an overlap matching is clear from its construction, and this concludes the proof.
\end{proof}
...
\longrightarrow
\bigoplus_{\substack{i, j \\ b_i \: = \: d_j}} \hom(K(a_i, b_i), K(c_j, d_j))
\hookrightarrow \hom(X, Y)
\]
on morphisms, where $X = \bigoplus_i K(a_i, b_i)$ and $Y = \bigoplus_j K(c_j, d_j)$. We call $\Delta$ the \textbf{same-endpoint operator}.
\end{definition}
...
\begin{proof}
Clearly $\Delta \id = \id$. For compatibility with composition, consider $f \colon \bigoplus_i K(a_i, b_i) \to \bigoplus_j K(c_j, d_j)$ and $g \colon \bigoplus_j K(c_j, d_j) \to \bigoplus_k K(s_k, t_k)$ composable morphisms. We write $f_{ij}$ for the corresponding morphism
\[
...
\begin{figure}[tbp]
\centering
% \includegraphics[width=0.95\textwidth]{Figures/bar-to-bar-example.pdf}
...
In \cite{Paraskevopoulos2012} the author presents a reduction algorithm for infinite row-finite matrices (defined analogously to column-finite ones). We proceed similarly with \cref{C!A:inf-red}: compare with the standard algorithm of persistent homology, see \cite{cohen2006vines}.
...
Recall that we use the the category $\Barc$ from \cite{bauer2020persistence}. While for their canonically induced matchings compose naturally, we need more structure on our persistence modules to make the matchings compose in an intuitive way.

\begin{definition}[Matching operators]
A \textbf{matching operator} is a function $\mu$ that takes a morphism $f \colon X \to Y$ in $\Bar$ and returns a matching (i.e., morphism) $\mu(f) \colon \cc{B}(X) \to \cc{B}(Y)$ in $\Barc$. The second argument is sometimes omitted. The operator $\mu$ is
\begin{enumerate}
\item \textbf{additive} if, given morphisms $f$ and $g$, $\mu(f \oplus g) = \mu(f) \sqcup \mu(g)$.
\item \textbf{split} if, for a strictly bar-to-bar morphism $f$, $\mu(f)$ is the matching of \cref{C!D:btb}\footnote{more precisely, the definition's matching is $\{(i, j) \mid ((\langle a_i, b_i \rangle, i), (\langle c_j, d_j \rangle, j)) \in \mu(f)\}$.}.
\item \textbf{functorial on monomorphisms} (resp.\ \textbf{epimorphisms}) if, given composable monomorphisms (resp.\ epimorphisms) $f$ and $g$, $\mu(g \circ f) = \mu(g) \bullet \mu(f)$.
\end{enumerate}
\end{definition}

\begin{remark}
\begin{enumerate}
\item Matching operators are well-defined. In particular, they are equivariant under reindexing of the barcode multiset representations \cite{bauer2020persistence}.
\item By \cite[Proposition 5.10]{bauer2015induced}, such operators are never functorial over all morphisms. Moreover, the \textbf{canonically induced matching} of that paper is functorial on monomorphisms and epimorphisms.
...
\item Using the axiom of choice, and viewing morphisms as objects in the additive category $\Fun(\R \times \{0 < 1\}, \Vect_K)$, we can define arbitrary additive matchings by choosing a matching for each indecomposable morphism.
\end{enumerate}
\end{remark}
...
\item The matching operator $\mu_\Delta$ is not very informative on arbitrary morphisms. Indeed, it is empty as soon as no bars of $X$ or $Y$ have the same end-point.
\end{enumerate}
\end{remark}

Instead, we may follow the strategy of \cite{bauer2015induced}: given a morphism $f \colon X \to Y$ in $\Bar$, consider an epi-mono factorization of $f$,
\[
...
\begin{proof}
First, we observe that $\im(f) \cong \im(\tilde{f})$. Next, we recall that we have assumed that $X$ and $Y$ have countably many bars. Moreover, the row labels (resp.\ column labels) of $\tilde{F}$ are totally ordered, as they all share the same startpoint (resp.\ endpoint). We can therefore apply \cref{C!A:labeled-red} with $\tilde{F}$ as input. By the assumption that the row and column labels are totally ordered, we know that all left-to-right column operations and bottom-to-top row operations on the matrix $\tilde{F}$ are valid matrix operations.
...
\end{cases}
\]
% \ag{($R$ is defined below by $M E = R \coloneq U^{-1} A$)}
...
\end{itemize}
We observe that the resulting $M$ is in reduced row echelon form, with pivots at the coordinates $(i, i)$ for $i$ in $\tilde{I}$. Each column as a pivot and the pivots' row and column labels have the same endpoint $d_i$, so we deduce that $M$ represents a monomorphism: indeed, at every time $t \in \R$, $M_t$ represents an injective linear map.
...
We first show the pairing uniqueness lemma \cite{cohen2006vines} for tame fringe presentations, following the reasoning and notations of the original proof. Given a matrix $A \in K^{m \times n}$, for all $i \in \{1, \ldots, m\}$ and $j \in \{1, \ldots, n\}$, denote by $A_i^j$ the lower left minor of $A$ obtained by removing the first $i - 1$ rows and the last $n - j$ columns. Define
\[
...
% {\color{violet}(Not so sure about this but this might be useful to show that we recover the Spanish algorithm) If we have the following configuration (with columns  ordered by birth then death and row by death then birth):
% \[
% F = \bordermatrix { 
%     & K(a_i,b_i) & K(a_s,b_s) & K(a_j,b_j)\cr 
%     K(c_i,d_i) & . & 1 & . \cr
%     K(c_s,d_s) & 1 & x & . \cr
%     K(c_j,d_j) & . & 1 & . \cr}
% \]
% we can show that $K(a_s,b_s)$ and $K(c_s,d_s)$ overlap and we have the same result if there exist a valid entry at each side of $x$ in the same row and one below in the same column. Moreover, if an unvalid nonzero $x$ entry is created it implies that there is a valid entry at the left of $x$ and below it. Therefore all the entries above and at the right of $x$ can not be valid. To conclude that the two algorithms give the same pivots we can apply the pairing uniqueness lemma to say that $FV_1$ and $FV_2$ have the same pivots (with possibly unvalid entries) then say that removing the unvalid entries does not change the pivots since all the entries above are also unvalid.}


% Now let $f$ be a morphism in $\Pfd$. Given a choice of representative $g$ in $\Bar$, we can define the induced matching $\mu(g)$. Recall that we consider matchings as multiset representations. The induced matching $\mu(g)$ induces a multiset matching $M(g)$, and we now show that this multiset matching does not depend on the choice of $g$.

% {\color{magenta}
% \begin{proposition}
% Let $f$ be a morphism in $\Pfd$ and $g$ a representative in $\Bar$. The induced multiset matching $M(g)$ does not depend on the choice of $g$.
% \end{proposition}

% \begin{proof}
% Let $g$ and $h \colon X \to Y$ be representatives of $f$ in $\Bar$, with $X$ and $Y$ the barcode decompositions of the domain and codomain of $f$, respectively. We have the following diagram:
% \[
% \begin{tikzcd}
% \tilde{X}
%   \arrow[r, two heads]
%   \arrow[d, "\tilde{\varphi}" swap]
% & X
%   \arrow[r, "g"]
%   \arrow[d, "\varphi" swap]
% & Y
...
% \end{tikzcd}
% \]
% where $\varphi \colon X \to X$ and $\psi \colon Y \to Y$ are the isomorphisms that relate $g$ and $h$ (all representatives of $f$ are isomorphic), $\tilde{X}$ and $\tilde{Y}$ are the persistence modules defined in \cref{C!P:image-barcode}, and $\tilde{\varphi}$ and $\tilde{\psi}$ are the induced maps on $\tilde{X}$ and $\tilde{Y}$.

% First, we show that $\tilde{\varphi}$ is an isomorphism. Denoting by $\Phi$ the ordered matrix representation of $\varphi$ whose rows and columns are ordered lexicographically by birth than death, and similarly writing $\tilde{\Phi}$, we have the following equivalences:
% \begin{itemize}
% \item $\varphi$ is an isomorphism.
% \item For all $t \in \R$, $\varphi_t \colon X_t \to X_t$ is an isomorphism.
% \item For all $t \in \R$, every same-startpoint block of $\Phi_t$ (the matrix $\Phi$ restricted to rows and columns whose label contains $t$) is invertible.
% \item For all $t \in \R$, every same-startpoint block of $\tilde{\Phi}_t$ is invertible.
% \item For all $t \in \R$, $\tilde{\varphi}_t$ is an isomorphism.
% \item $\tilde{\varphi}$ is an isomorphism.
% \end{itemize}
% Thus $\tilde{\varphi}$ is an isomorphism. We reason similarly for $\tilde{\psi}$, ordering the matrix labels by death than birth, and using same-endpoint blocks instead of same-startpoint blocks. We deduce that $\tilde{g}$ and $\tilde{h}$ are isomorphic.

% Similarly to \cref{C!P:unique-induced-matching}, we can view the morphisms $\tilde{g}$ and $\tilde{h}$ as objects in the category $\Fun(\R \times \{0 < 1 \}, \Vect_K)$. Applying \cref{C!A:labeled-red}, we deduce that these objects decompose into simple indecomposables (compare also with \cite[Theorem 5.3]{jacquard2023space}). As this decomposition is unique, we conclude that the pivots of $\tilde{G}$ and $\tilde{H}$ are the same up to permutation of identically labeled bars.

% \textcolor{red}{(Isaac) continue from here, but I'm not sure if this is going to lead to something correct. The issue is that I don't know how to guarantee that these permutations are only between bars that were originally identical in $X$ and $Y$.}
% \end{proof}
% }

% {\color{lightgray}
% \begin{lemma}
% \label{C!L:bar-permutation}
% Let $F$ be a possibly countably infinite, column-finite matrix, and let $\Phi$ be a matrix that permutes two adjacent columns when multiplied on the right. Given the output $A = U F V$ of \cref{C!A:labeled-red} applied to the matrix $F$, there exist $U'$, $V'$ upper triangular invertible such that $A' \coloneq U' F \Phi V'$ is equal to $A$ up to permutation of identically labeled bars.
% \end{lemma}

% \begin{proof}
% Consider the execution of \cref{C!A:labeled-red} on $F$. In particular, we can write
% \[
% V = \prod_{j' < j} (I - \lambda_{j', j} E_{j', j}),
% \]
% where the order in which we multiply matrices is given by the algorithm and $\lambda_{j', j} = 0$ if we do not add $A_{*, j'}$ to $A_{*, j}$. This notation is well defined because each pair of columns $(j', j)$ is only considered once in the algorithm.

% Denote by $j_0$ and $j_0 + 1$ the indices of the two permuted columns. If we never add column $j_0$ to column $j_0 + 1$, then we set
% \begin{equation}
% \label{C!Eq:v-prime}
% V' \coloneq \prod_{j' < j} (I - \lambda_{j', j} E_{\sigma(j'), \sigma(j)}),
% \end{equation}
% where $\sigma$ is the permutation $(j_0 \; j_0 + 1)$ on $J$. Every factor of this product is upper triangular and invertible, so $V'$ is so as well. We also show (by induction on the product defining $V'$) that $U F \Phi V'$ is equal to $A$ with columns $j_0$ and $j_0 + 1$ switched.

% Now suppose that, at some point in the execution of the algorithm, we do add column $j_0$ to column $j_0 + 1$. Then we know that this happens only once, when the for loop is at index $j_0 + 1$. We define $V'$ as in \eqref{C!Eq:v-prime} except that
% \begin{itemize}
% \item every factor of the form $(I - \lambda_{j_0, j} E_{\sigma(j_0), \sigma(j)})$ with $j > j_0 + 1$ is replaced by $(I - \lambda_{j_0, j} E_{j_0 + 1, j} + \lambda_{j_0, j} E_{j_0, j})$ (note that $\sigma(j) = j$);
% \item every factor of the form $(I - \lambda_{j_0 + 1, j} E_{\sigma(j_0 + 1), \sigma(j)})$ with $j > j_0 + 1$ is replaced by $(I - \lambda_{j_0 + 1, j} E_{j_0 + 1, j})$.
% \end{itemize}
% By this construction, we can show by induction on the product defining $V'$ that $A'' \coloneq U F \Phi V'$ has the same pivots as $A$. Moreover, the only part of the matrix $A''$ that does not have at most one nonzero coefficient on each row and column is column $j_0$. We can therefore multiply $U$ with some more row reduction operations to reduce $A''$ to $A'$, equal to $A$. This concludes the proof.
% \end{proof}

% \begin{lemma}
% \label{C!L:extract-permutation}
% Let $\varphi \colon X \to X$ be an isomorphism in $\Bar$ and $\Phi$ an ordered matrix representation whose columns and rows are either both ordered lexicographically by birth than death, or both ordered lexicographically by death than birth. Then $\Phi$ is equal to $U \Sigma V$, where $\Sigma$ is a permutation matrix (exactly one nonzero coefficient in each row and column) and $U$ and $V$ are invertible upper triangular.
% \end{lemma}

% \begin{proof}
% We assume that the rows and columns of $\Phi$ are ordered lexicographically by birth than death; the other case is handled analogously. Consider the application of \cref{C!A:invertible-reduction} to $\Phi$. The algorithm is properly defined; we check some aspects here.
% \begin{itemize}
% \item In the first while loop, column operations only happen between columns with the same startpoint. Indeed, the block in $\Phi$ whose rows and columns are labeled with a given startpoint is a diagonal block of $\Phi$. Since $\Phi$ is invertible, this block must be of full rank, and so the lowest nonzero element of a column in this block must belong to a row in the block. Moreover, no column to the left of this block may have a lowest nonzero coefficient on a row of this block, as that column's label would not overlap above this block's rows' labels. In this block, the endpoints are in increasing order, so all operations are valid: in particular, nonzero coefficients are always between overlapping bars.
% \item In the second while loop, by design, the row operations only happen between rows with the same endpoint. Once again, since the startpoints in this block are in increasing order, all operations are valid.
% \item In the third while loop, the column index $j'$ always exists because $\varphi$ is an isomorphism. Moreover, this $j'$ column only has one nonzero coefficient, so the column operation is valid.
% \end{itemize}
% The convergence of this algorithm can be verified as in the proof of \cref{C!L:good-alg}.

% From the output of \cref{C!A:invertible-reduction}, we set $\Sigma \coloneq A$ and invert $U$ and $V$ to obtain $\Phi = U \Sigma V$, and this concludes the proof.
% \end{proof}

% \begin{algorithm}[tbp]
% \caption{Reduction of invertible matrix representations}
% \label{C!A:invertible-reduction}
% \KwIn{an invertible matrix representation $M$ whose rows and columns are ordered by birth than death.}
% \KwOut{a factorization $A = U M V$, where $A$ is a permutation matrix and $U$ and $V$ are invertible upper triangular.}
...
%   }
%   $i \leftarrow \low(j)$\;
%   $i_0 \leftarrow \min\{i' \in I \mid a_{i'} = a_i\}$ \tcp*{highest row with same startpoint}
%   \While(\tcp*[f]{reduce nonzero entries above low in same-start block}){$\exists i' \in \{i_0, \ldots, i - 1\}, \; A_{i', j} \neq 0$}{%
%     $\lambda \leftarrow A_{i', j} / A_{i, j}$\;
%     $A \leftarrow (I - \lambda E_{i', i}) A$\; 
%     $U \leftarrow (I - \lambda E_{i', i}) U$\;
%   }
%   \While(\tcp*[f]{reduce nonzero entries above same-end block}){$\exists i' < i_0, \; A_{i', j} \neq 0$}{%
%     Find $j'$ such that $\low(j') = i'$\;
%     $\lambda \leftarrow A_{i', j} / A_{i, j}$\;
%     $A \leftarrow A (I - \lambda E_{j', j})$ \ir{Why is this valid?}\;
%     $V \leftarrow V (I - \lambda E_{j', j})$\;
%   }
% }
% \Return{$A$, $U$, $V$}
...
% \begin{remark}
% At first, one might think that we can apply \cref{C!A:labeled-red} to $\Phi$ to get the reduced matrix $A$. However, this is not possible in general, as the rows and columns of $\Phi$ are labeled by arbitrary intervals that may not be totally orderable. This is why we introduce \cref{C!A:invertible-reduction}.
% \end{remark}

% \begin{proposition}
% Let $f \colon X \to Y$ be a morphism in $\Pfd$ and $g$ a representative in $\Bar$. The induced multiset matching $M(g)$ does not depend on the choice of $g$.
% \end{proposition}

% \begin{proof}
% \ir{(Incredibly confusingly written; I will go over this again if I have time)}

% Let $g$ and $h$ be representatives of $f$ in $\Bar$ and let $G$ and $H$ be ordered matrix representatives of $g$ and $h$, respectively. Then there exist invertible (labeled) matrices $\Psi$ and $\Phi$ such that $H = \Psi G \Phi$. By \cref{C!R:knowing-epi-suffices}, it suffices to show that \cref{C!A:labeled-red} applied to $\tilde{G}$ and $\tilde{H}$ (see \cref{C!P:image-barcode}) return matrices that are equal up to permutation of rows and columns whose original labels (i.e.\ in $G$ and $H$) are identically labeled, since this will lead to identical induced multiset matchings.

% Let us show that $\tilde{G}$ and $\tilde{H}$ have the same pivots up to permutation of rows and columns with identical original labels (in $G$ and $H$), which implies that their reductions by \cref{C!A:labeled-red} are equal, up to said permutation. By \cref{C!L:extract-permutation}, there exist permutation matrices $\Sigma$ and $\Pi$ and upper triangular invertible matrices $U$, $V$, $U'$ and $V'$ such that $\Phi = U \Sigma V$ and $\Psi = U' \Pi V'$. Denote by $\tilde{U}$, $\tilde{\Sigma}$, and $\tilde{V}$ the relabelings of the matrices $U$, $\Sigma$, and $V$ where we replace each label $\langle a, b \rangle$ with $\langle a, \infty \rangle$. Denote by $\tilde{U}'$, $\tilde{\Pi}$, and $\tilde{V}'$ the relabelings of the matrices $U'$, $\Pi$, and $V'$ where we replace each label $\langle c, d \rangle$ with $\langle -\infty, d \rangle$. We check that the following equality of relabeled matrices holds:
% \[
% \tilde{H} = \tilde{U}' \tilde{\Pi} \tilde{V}' \tilde{G} \tilde{U} \tilde{\Sigma} \tilde{V}.
% \]
% Indeed, on the righthand side, a coefficient with column label $\langle a, \infty \rangle$ and row label $\langle -\infty, d \rangle$ is nonzero if, and only if, the coefficient in the original overlap multiplication, whose column label is $\langle a, b \rangle$ and row label is $\langle c, d \rangle$, is nonzero. \ir{In more details: in the original product $H = U' \Pi V' G U \Sigma V$, each coefficient can be expressed as the sum of a product of coefficients where the labels associated to each coefficient are properly overlapping. The only way for a label to not be properly overlapping is if they are disjoint, with $d < a$, and in this case the extended intervals are not overlapping either. Thus we check that the tilde-fied product has the exact same coefficients as the original product.}
% Note also that $\tilde{\Pi}$ and $\tilde{\Sigma}$ are permutation matrices that only permute rows and columns whose original labels (in $\Pi$ and $\Sigma$) are identical.

% Multiplying a matrix by an upper triangular matrix does not change the matrix's pivots \ir{(Prove this for elementary column and row operations and then conclude by induction)}, so we can ignore $\tilde{U}'$ and $\tilde{V}$. By \cref{C!L:bar-permutation} and induction on the decompositions of $\Pi$ and $\Sigma$ into permutations of adjacent rows and columns, we know that $\tilde{V}' \tilde{G} \tilde{U}$ has the same pivots as $\tilde{\Pi} \tilde{V}' \tilde{G} \tilde{U} \tilde{\Sigma}$ up to permutation of rows and columns whose original labels are identical. Therefore we can ignore $\tilde{\Pi}$ and $\tilde{\Sigma}$. Finally, as for $\tilde{U}'$ and $\tilde{V}$, we can ignore $\tilde{V}'$ and $\tilde{U}$ to conclude that $\tilde{G}$ and $\tilde{H}$ have the same pivots up to permutation of rows and columns with identical original labels. This concludes the proof.
% \end{proof}
% }

% We can therefore define the \textbf{induced multiset matching} $M(f)$ for all morphisms $f$ in $\Pfd$, and this is a \ir{(alternate?)} proof that the $\cc{P}$-matching defined in \cite{GST2025} is unique \ir{i.e., does not depend on the choice of bases}.
...
In this section we prove some results in the category $\Pfd$. In particular, we highlight a connection between rank functions and $p$-norms, which yields new proofs of results in \cite[Section 3]{AGRS2025}.
...
\quad \text{and} \quad
\{i \in \{1, \ldots, n\} : c_i \leq s^-\} = \{1, \ldots, k_c\}.
\]
Therefore
\[
\{i \in \{1, \ldots, n\} : c_i \leq s^-\} \setminus \{i \in \{1, \ldots, m\} : a_i \leq t^-\} = \begin{cases}
...
\begin{align*}
\rk((\coker f_\varphi)_{s \leq t}) & = |\{i \in \{1, \ldots, m\} : c_i \leq s^-, t^- < a_i\} \sqcup \{j \in \{m + 1, \ldots, n\} : c_i \leq s^-\}| \\
...
& = (\rk Y_{s \leq t} - \dim X_t)_+ \\
& = \rk((\coker f_\varphi)_{s \leq t}). \tag{by \cref{C!L:canonical-coker}}
\end{align*}
\end{proof}
...
When $p=1$, as observed for example in \cite{bubenik2022exact}, the $1$-norm $\| X \|_1$ of a persistence module coincides with the integral $\int_{\R} \dim X_{t} \textnormal{d}t$ of its Hilbert function $(\dim X)(t)\coloneqq \dim X_t$. 
\end{remark}
...
\begin{remark}
When $p=1$, the inequalities of Theorem \ref{C!thm:cost-coker-btb} are equalities: $\| \coker f \|_1 = \| \coker \Delta f \|_1 = \| \coker f_\varphi \|_1$. To see this, recall Remark \ref{C!rmk:int-norm1} and observe that $\dim (\coker f)_t = \dim (\coker \Delta f)_t = \dim (\coker f_\varphi)_t$ since each of these dimensions equals $\dim Y_t - \dim X_t$. For any $p>1$, it is easy to find a monomorphism $f \colon X \hookrightarrow Y$ such that the inequalities of Theorem \ref{C!thm:cost-coker-btb} are strict.    
\end{remark}
...
We now provide alternate proofs of the construction of algebraic $p$-Wasserstein distances and of the isometry result for $p$-Wasserstein distances \cite{skraba2020wasserstein}. We define algebraic $p$-Wasserstein distances by defining a noise system \cite{scolamiero2017multidimensional}: see \cite{AGRS2025} for the details to this approach in the context of \textit{tame} persistence modules. In particular, we need to show the following result.
...
\begin{proposition}\label{C!prop:rkYge}
...
\begin{corollary}[{\cite[Remark 7.32]{skraba2020wasserstein}}]
...
We provide an alternate (for one direction at least) proof to \cite[Theorems 7.27-7.28]{skraba2020wasserstein}, the isometry result between algebraic and combinatorial $p$-Wasserstein distances.
...
\begin{proof}
\noindent\fbox{$\le$}
Let $\varphi \colon \mathsf{Dgm}(X) \to \mathsf{Dgm}(Y)$ be a matching. We want to construct a span with a smaller or equal cost. Following \cite[Theorem 7.28]{skraba2020wasserstein}, define
\[
