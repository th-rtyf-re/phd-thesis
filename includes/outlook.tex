...
\begin{itemize}
  \item[monotonicity] $\rank V(a \leq d) \leq \rank V(b \leq c)$, and
  \item[chain-supermodularity] $\rank V(a \leq d) + \rank V(b \leq c) \leq \rank V(a \leq c) + \rank V(b \leq d)$.
\end{itemize}
...
It is still an open problem to determine the extension with largest $p$-norm.
