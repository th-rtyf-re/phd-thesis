...
In this thesis we explored many ways in which algebraic invariants for filtered spaces can be defined and computed. There is still much more to investigate, and we present some ongoing work. We freely use the notations and results of the included papers. \ir{This section is in draft form}
...
The rank function of a persistence module over a poset $I$ satisfies some inequalities \ir{list them}. These inequalities have been studied in other contexts under the name \term{supermodularity} \ir{cite}. We aim to study the space of \term{chain-supermodular} functions.
...
Following Papers \ref{PC:AGRS2025} and \ref{PC:GRRS2026}, we want to define a noise system using this $p$-norm. In other words, we want to check that the $p$-norm defines an amplitude. The first, easier short exact sequence axiom (see \cref{D:noise-system}) comes from [Paper \ref{PC:GRRS2026}, Proposition 4.9], as in the one-dimensional case.
...
Let $V$ and $W$ be persistence modules. An \term[extension]{extension of $V$ and $W$} is a persistence module $E$ that fits into a short exact sequence
\[
  0 \to V \to E \to W \to 0.
\]
The question that Hugo Åkesson studied in his Master's thesis is: what are the maximal and minimal $p$-norms of an extension? We have partial results if $V$ or $W$ is a single bar:

\begin{proposition}
  \ir{to do}
\end{proposition}

Studying this problems also gives us insight into the space of extensions. The extension barcodes can be parameterized by antichains.
