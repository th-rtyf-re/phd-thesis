...
\subsection{Persistence generators}

Free module.

Generators can be: elements of specific vector spaces $V_t$, morphisms from free module to $V$.

A set of generators generates $V$ if every element is a sum of generators up to transition maps.

A basis is an isomorphism to $\Bar$.

Remark: this theory can be expanded to linear independence, but we do not need that.

\subsection{Matrix representations}

Just as there are many ways of looking at persistence modules, there are also many ways of looking at morphisms of persistence modules. In this section, we consider the matrix perspective. We mainly work in the one-dimensional case.

We first define a category, equivalent to $\Pfd$, where we can work explicitly with bars.

\begin{definition}
  the category $\Bar$. Objects are explicit direct sums!
\end{definition}

The idea behind representing morphisms of persistence modules as matrices comes from the isomorphism
\[
\hom(\bigoplus_i \bf{k}_{S_i}, \bigoplus_j \bf{k}_{T_j}) \cong \prod_{i, j} \hom(\bf{k}_{S_i}, \bf{k}_{T_j}).
\]
The isomorphism holds for all abelian categories by the universal properties of direct sums as products and coproducts.

\begin{remark}
  In $\Tame$, the product is a direct sum.
  
  Example where product is not direct sum: $\K_{(0, 1)} \to \bigoplus_{n \geq 1} \K_{(0, \frac{1}{n})}$.
\end{remark}

\begin{definition}
  (\term[(ordered) matrix representation]{ordered}) \textbf{matrix representations}. Rows and columns should be labeled. The big question is how to label them, and how to order the rows and columns.
  
  Matrix multiplication: it is nontrivial.
\end{definition}

\begin{remark}
  There is an alternate interpretation of matrices using persistence vectors. Matrix multiplication is slightly different. \ir{Maybe just briefly mention this.}
\end{remark}

\begin{remark}
  This matrix theory also works for general persistence modules, if we label by indecomposables... example: free modules.
\end{remark}

\subsection{The pairing uniqueness lemma}

We can view boundary map as morphism of persistence modules: since persistent homology is computed pointwise, we can define persistent homology as the homology of persistent chain complexes, which are just sequences of persistence modules with differential maps. We describe an algorithm for computing persistent homology:

...

We can view this as a morphism between free modules.

The isomorphism of the barcode decomposition theorem can be explicitly computed by reducing a matrix appropriately (morphism of free resolution). Key result: Pivots do not depend on order of operations.

% ====

\section{Normed persistent algebra}

Back to one-parameter: we have already introduced some notions of metrics (and by extension, norms) of persistence modules.

\subsection{Bar-to-bar morphisms and algebraic Wasserstein distances}

Define $p$-norms, algebraic Wasserstein distances. Key point is size of cokernels of monomorphisms (kernels of epimorphisms).
...
In this section, we consider persistence modules as functors over posets.
...
Decomposition of rank functions? (if not detailed earlier)
