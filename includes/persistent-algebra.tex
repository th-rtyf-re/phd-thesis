...
\label{D:category}
  ...
  \end{itemize}
  \ir{Define natural transformations. A morphism of functors is a natural transformation. We write $f_t$ for $t \in I$. Define equivalence of categories.}
\end{definition}
...
\label{D:persistence-module}
  Fix a field $\bf{k}$ and $(I, \mathord{\leq})$ a totally ordered set. A \term[persistence module]{persistence module over $I$} is a collection of $\bf{k}$-vector spaces $(V_i)_{i \in I}$ indexed by $I$, along with linear maps $(V_{i \leq j} \colon V_i \to V_j)_{i \leq j \in I}$, called \term{transition maps}, such that
  \begin{itemize}
...
\begin{definition**}
  ...
  \end{enumerate}
  Now let us equip our poset $(I, \mathord{\leq})$ with a topology. The \term{Alexandrov topology} on $I$ is the topology where open sets are \term[downset]{downsets}, that is, subsets $S$ of $I$ such that $S = \{y \in I \mid \exists x \in S, \; y \leq x\}$. We check that this is indeed a topology.
\end{definition**}
...
\begin{definition}
\label{D:interval}
  Let $(I, \mathord{\leq})$ be a poset. An \term{interval} of $I$ is a subset $S$ such that, for all, $i, j$ in $S$ and $k$ in $I$ such that $i \leq k \leq j$, $k$ is also an element of $S$.
  ...
  A \term{pointwise finite-dimensional persistence module} is a persistence module $V$ indexed by $(I, \mathord{\leq})$ such that, for all $i \in I$, $V(i)$ is a finite-dimensional vector space.
\end{definition}
...
\begin{definition**}
  A \term{persistence set} is a functor from a poset $(I, \mathord{\leq})$, seen as a category, to the category of sets $\Set$. More explicitly, it is the given of a set $S(i)$ for all $i$ in $I$ and transition maps $S(i \leq j) \colon S(i) \to S(j)$ for all $i \leq j$ in $I$ such that, for all $i \leq j \leq k$ in $I$, $S(j \leq k) \circ S(i \leq j) = S(i \leq k)$.
\end{definition**}
...
We now return to the one-dimensional case.

\ir{Not sure how to structure this. Noise systems aren't a focus of this thesis, so they can go in the 13Ways part. But algebraic Wasserstein is pretty relevant, so it should be highlighted here? Along with induced matchings? But this could also go in the contributions section...}
