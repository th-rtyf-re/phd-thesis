...
% Subtle optimizations in layout
\usepackage{microtype}
% Paragraphs start with vertical space, not indent
\usepackage{parskip}
...
% thmtools for QED in any theorem
\usepackage{amsthm,thmtools}
% Labeled matrices with square brackets
\usepackage{blkarray}
...
\usepackage[
  ...
  bookmarksdepth=2,% w/o argument: uses tocdepth, supposedly
  pdfusetitle,
  colorlinks,
  linktoc=all,
...
% Improved font with ligatures (font needs to be installed separately)
...
\addbibresource{bib/btb.bib}
\addbibresource{bib/gM.bib}
...
\allowdisplaybreaks  % equations can go over multiple pages
...
\fancypagestyle{front-fancy}{% for special pages, e.g. first pages of chapters
  \fancyhead{}  % reset
  \fancyhead[LE,RO]{\sffamily\small\thepage}
}

\fancypagestyle{paper-fancy}{
  \fancyhead{}
  \fancyhead[LE]{%
    \makebox[0pt][l]{\hspace{-5mm}\textcolor{white}{\rule[-2pt]{25mm}{15pt}}}%
    \sffamily\small\thepage%
  }
  \fancyhead[RO]{%
    \makebox[20mm][l]{\textcolor{white}{\rule[-2pt]{30mm}{15pt}}}%
    \sffamily\small\thepage%
  }
}
...
\fancyfoot[RO]{%
  \AddToHookNext{shipout/background}{
    \ifnum\ReadonlyShipoutCounter<88\relax% Manually adjusted max page number
      % Manually adjusted positioning
      \put(\paperwidth-30mm,3mm-\paperheight){\input{figs/flip/leo-\fpeval{trunc(\ReadonlyShipoutCounter / 2, 0)}.pgf}}%
    \fi
  }
}
...
\def\frontmatter{\oldfrontmatter\pagestyle{front-fancy}}
\def\mainmatter{\oldmainmatter\pagestyle{fancy}}
\def\backmatter{\oldbackmatter}
\makeatother
...
\RenewCommandCopy{\theHtheorem}{\thetheorem}  % fix issue with thmtools and hyperref
...
\SetAlFnt{\small}
\SetAlCapFnt{\small}  % This didn't work... it's in the official version of the thesis, so I'm keeping this here.
